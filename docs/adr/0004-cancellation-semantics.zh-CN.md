...
- 修订关系: Agent::with_timeout 为显式 opt-in，不属于本文否决的
  "框架内建默认超时"（ADR-0015 决策九澄清）；agent 级默认 +
  run 级覆盖的双层超时语义随 ADR-0014 实施重审
- 评审修订（2026-09-09，v4 实施期）：取消入口的**命名立法**——
  外部取消信号参数更名为 `cancel_token`（与类型 `CancellationToken`
  指代对应，原 `token` 指代不明）；框架内部共享状态变量统一命名
  `cancel_core`（原裸 `core` 指代丢失）。四入口立法本身不变。
- 日期: 2026-08-28
- 决策者: irylex（人类确认）

...
| M11 | ⏳ 待实施 | — | 记忆与上下文系统（ADR-0012，原 S2b/S2c 溶解）：SynonzRuntime（显式 Bootstrap+注册表+缺省默认）+ Subject 一等实体 + Conversation::new/of(&runtime, &subject) 工厂族 + 分层记忆（L1/L2 会话内、L3 跨会话、片段三元组）+ MemoryStore/ConversationStore/ContextAssembly 三兄弟契约 + 触发策略体系（TurnCount/L2Overflow 强制保底+事件策略叠加）+ 主题状态机 + 组装管线（每次 ask 新鲜装配、ContextManagement 事件兑现）。破坏性：Conversation::new 签名变更 |

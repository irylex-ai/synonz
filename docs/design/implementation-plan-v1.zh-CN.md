...
| S2a | ⏳ 待实施 | — | 会话实体（ADR-0011）：Turn/TurnInput/Conversation（id/turn_input/push_turn/messages/turns/truncate/export/import serde）；ask/run 入参改 impl Into<TurnInput>；执行体收尾落账（仅 Completed 入史）。验收：多轮记忆/落账时序/多 Agent 续会话/恢复往返测试 |

...
| M10 | ✅ 完成 | 2026-08-29 | API 演进（ADR-0011 决策 10，破坏性 pre-1.0 标注）：RunStream→Run 重命名；Run 双面（Stream<AgentEvent> + Future→Result<AgentOutput>，终止事件缓存支持先迭代后 await）；Answer 新类型（Stream<ModelDelta> + Future，流式优先）；ask 语义翻转（ask().await? 零破坏验证）；cancel() 显式方法（四入口汇一信号）；agent.with_timeout 默认预算。验收：既有 89 测试全绿 + 7 句柄测试（流式+await 双面/交错/取消映射/链式超时/默认预算） |
| S2a | ✅ 完成 | 2026-08-29 | 会话实体（ADR-0011）：Turn/TurnInput/Conversation（id/turn_input/push_turn/fork/messages/turns/truncate/export/import serde；Arc 共享语义——执行体写入用户同一会话）；ask/run 入参 impl Into<TurnInput<'_>>（三来源）；执行体收尾落账（仅 Completed 入史）；Answer/Run 持有 &Conversation 借用守卫（类型系统串行化轮次）；7 会话测试（多轮记忆/扁平历史回放/取消与失败不入史/多 Agent 续会话/一次性/手工管理） |
| M11 | ✅ 完成 | 2026-08-29 | 记忆与上下文系统（ADR-0012）分三步：M11a SynonzRuntime（显式 Bootstrap+注册表+缺省默认）/ Subject（(SubjectType,id) 二元组）/ Conversation 工厂族（new/of/with_id+自动保存）；M11b 触发引擎（run_post_turn_flows：L1 写入/主题更新/TurnCount 降级含 ContextManagement 可见摘要/L2Overflow 蒸馏）+ 主题检测器契约 + Conversation::end + sweep_stale 空闲兜底；M11c ContextAssembly 契约（LayeredMemory 默认/ConversationHistory 收编）+ Context 会话级 Runtime + Agent::with_context + 每次 ask 新鲜装配 + 落账后记忆流（Completed 前）。破坏性：Conversation::new 签名变更。120 测试全绿 |
